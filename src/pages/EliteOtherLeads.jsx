...
export default function EliteOtherLeads() {
    ...
    const referredById = eliteAmbassador.find(
        item => item.id === eliteAmbassadorId
    )?.referredByUid
    ...
    const filteredMyLeads = useMemo(() => {
        ...
        list = list.filter((l) => l.categoryId !== '-Os1EruiNYLx2XjzRUdF' && l?.eliteAmbassadorId === profile?.uid );
        const sorted = [...list]
        return sorted
    }, [
        leads,
    ...
    function openEdit(lead) {
        ...
        const cityName =
            String(lead.cityCode ?? lead.city ?? '').trim() ||
    ...
    async function saveLead(e) {
        ...
        const payload = {
            ...
            eliteAmbassadorId: eliteAmbassadorId,
            updatedAt: Date.now(),
        }
        if (editingId) {    
