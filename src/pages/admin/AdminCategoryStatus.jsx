...
export default function AdminCategoryStatus() {
  const { user, profile } = useAuth()
  const { categoryStatus, loading, error } = useCategoryStatus()
  ...
  const categoryTable = useMemo(() => categoryStatus ?? [], [categoryStatus])
  ...
  async function handleCreate(e) {
    ...
    if (!name) {
      setFormError('Category Status is required.')
      return
    }

    setSubmitting(true)
    try {
      const newRef = push(ref(db, 'category_status'))
      await set(newRef, {
        name,
        createdAt: Date.now(),
      ...
      setCategoryName('')
      setMessage('Category Status added.')
    } catch (err) {
      setFormError(err?.message ?? 'Could not add category statuses.')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  async function handleDelete(categoryId, categoryLabel) {
    ...
    setFormError('')
    if (!isAdmin) {
      setFormError('Only admin can delete category statuses.')
      return
    }
    const ok = window.confirm(
      `Delete category status "${categoryLabel || categoryId}"? This cannot be undone.`,
    )
    if (!ok) return

    setDeletingCategoryId(categoryId)
    try {
      await remove(ref(db, `category_status/${categoryId}`))
      setMessage('Category Status deleted.')
    } catch (err) {
      setFormError(err?.message ?? 'Could not delete category status.')
    } finally {
  ...
  return (
    <div className="min-w-0 space-y-6">
      <div>
        <h1 className="text-2xl font-semibold text-white">Category Status master</h1>
        <p className="mt-1 text-sm text-slate-400">
          Admin adds category status here
        </p>
      </div>
      ...
      <section className="rounded-xl border border-slate-800 bg-slate-900/50 p-5">
        <h2 className="text-lg font-medium text-white">Add category status</h2>
        ...
        <form onSubmit={handleCreate} className="mt-4 grid gap-4 md:grid-cols-2">
          ...
          <div className="md:col-span-2 flex flex-wrap items-center justify-end gap-3">
            ...
            >
              {submitting ? 'Adding...' : 'Add category Status'}
            </button>
          </div>
        </form>
      </section>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-180 table-auto text-left text-xs sm:text-sm">
            <thead className="border-b border-slate-800 bg-slate-900/80 text-xs uppercase text-slate-500">
              <tr>
                <th className="px-4 py-3 font-medium whitespace-nowrap">Category Status</th>
