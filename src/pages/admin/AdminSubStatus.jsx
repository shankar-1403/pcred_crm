...
export default function AdminSubStatus() {
    const { user, profile } = useAuth()
    const { subStatus, loading, error } = useSubStatus()
    ...
    const serviceTable = useMemo(() => subStatus ?? [], [subStatus])

    function categoryName(id) {
        const categoryData = categoryStatus.find((data)=> data.id === id)
    ...
    async function handleCreate(e) {
        ...
        if (!name) {
            setFormError('Sub Status is required.')
            return
        }

        setSubmitting(true)
        try {
        const newRef = push(ref(db, 'sub_status'))
        await set(newRef, {
            name,
            category,
            createdAt: Date.now(),
        ...
        setServiceName('')
        setCategoryId('')
        setMessage('Sub Status added.')
        } catch (err) {
        setFormError(err?.message ?? 'Could not add sub status.')
        } finally {
        setSubmitting(false)
        }
    }
    ...
    async function handleDelete(serviceId, serviceLabel) {
        ...
        setFormError('')
        if (!isAdmin) {
        setFormError('Only admin can delete sub status.')
        return
        }
        const ok = window.confirm(
        `Delete sub status "${serviceLabel || serviceId}"? This cannot be undone.`,
        )
        if (!ok) return

        setDeletingServiceId(serviceId)
        try {
        await remove(ref(db, `sub_status/${serviceId}`))
        setMessage('Sub Status deleted.')
        } catch (err) {
        setFormError(err?.message ?? 'Could not delete sub status.')
        } finally {
  ...
  return (
    <div className="min-w-0 space-y-6">
        <div>
            <h1 className="text-2xl font-semibold text-white">Sub Status master</h1>
            <p className="mt-1 text-sm text-slate-400">
            Admin adds sub status here
            </p>
        </div>
        ...
        <section className="rounded-xl border border-slate-800 bg-slate-900/50 p-5">
            <h2 className="text-lg font-medium text-white">Add Sub Status</h2>

            <form onSubmit={handleCreate} className="mt-4 grid gap-4 md:grid-cols-2">
                <div className="min-w-0">
                    <label className="block text-sm font-medium text-slate-300">Sub Status name</label>
                    <input
                        type="text"
                        required
                        value={serviceName}
                ...
                <div className="md:col-span-2 flex flex-wrap items-center justify-end gap-3">
                    ...
                    <button type="submit" disabled={!isAdmin || submitting} className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-500 disabled:opacity-50">
                        {submitting ? 'Adding...' : 'Add Sub Status'}
                    </button>
                </div>
            </form>
        </section>
